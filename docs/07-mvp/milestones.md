...
| **M3 — Persistence** | sqlite backend + restart recovery + jsonl replay | DCN survives restart; `COATr replay` reproduces a turn | ✅ — see [`README.md`](../../README.md) M3 row for per-PR breakdown |
| **M4 — Daemon + CLI** | Daemon HTTP/JSON-RPC + `COATr` CLI + host-sdk HTTP transport | Host calls daemon over socket and completes a turn | ✅ — see [`README.md`](../../README.md) M4 row for per-PR breakdown |
