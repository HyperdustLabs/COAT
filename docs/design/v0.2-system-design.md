<p align="center">
  <img src="../moss-coat.png" alt="OpenCOAT Runtime logo" width="160" />
</p>
...
三种拓扑共用同一份 `opencoat-runtime-core`，差别仅在 transport 与 daemon 是否独立运行。
...
| Package | 职责 | 依赖 |
| --- | --- | --- |
| `opencoat-runtime-protocol` | JSON Schema、OpenAPI、（可选）proto，类型生成 | 无 |
| `opencoat-runtime-core` | L2 全部纯逻辑模块、Ports 抽象 | protocol |
| `opencoat-runtime-storage` | ConcernStore / DCNStore 多后端实现 | core, protocol |
| `opencoat-runtime-llm` | LLM / Embedder 客户端 | core |
| `opencoat-runtime-host-sdk` | Host 端 SDK（emitter、consumer、装饰器、transport） | protocol |
| `opencoat-runtime-daemon` | 长驻进程：调度器、worker、IPC 服务 | core, storage, llm |
| `opencoat-runtime-cli` | `COATr` 命令行：起停、巡检、replay、可视化 | daemon, core |
| `opencoat-runtime-host-plugins/*` | 一方 host adapter（OpenClaw、Hermes、LangGraph、AutoGen、CrewAI、custom 模板） | host-sdk |
