...
``Daemon`` owns the runtime lifecycle for the long-running ``COATR``
process:
...
logger = logging.getLogger("COAT_runtime_daemon")
...
class Daemon:
    """Composes a :class:`COATRuntime` with IPC + signal-driven lifecycle.
    ...
    def start(self) -> None:
        ...
        with self._state_lock:
            ...
            try:
                ...
                logger.info(
                    "COAT daemon started (llm=%s, http=%s)",
    ...
    def reload(self) -> None:
        ...
        with self._state_lock:
            ...
            logger.info("COAT daemon reloaded (llm=%s)", new_built.llm_label)
