...
`opencoat-runtime-core`. The runtime never calls a provider directly —
...
├── stub_client.py           # re-export of opencoat_runtime_core.llm.StubLLMClient
...
| In-proc stub | `StubLLMClient` (lives in `opencoat_runtime_core.llm`) | scripted return value | n/a | scripted return value | none |
...
test suite lives in `opencoat_runtime_core.llm.StubLLMClient` and is
