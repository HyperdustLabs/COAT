"""Tests for :class:`~opencoat_runtime_daemon._pidfile.PidFile` (M4 PR-20)."""
...
import pytest
from opencoat_runtime_daemon._pidfile import PidFile, PidFileError


def test_acquire_writes_our_pid(tmp_path: Path) -> None:
    pf = PidFile(tmp_path / "opencoat.pid")
    pf.acquire()
    try:
...
def test_acquire_creates_parent_dirs(tmp_path: Path) -> None:
    p = tmp_path / "nested" / "run" / "opencoat.pid"
    with PidFile(p):
        assert p.exists()
    assert not p.exists()
