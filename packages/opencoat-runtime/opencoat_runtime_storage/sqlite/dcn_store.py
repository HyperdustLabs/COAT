...
pointing at ``/var/lib/opencoat/dcn.db`` doesn't have to ``mkdir -p``
themselves.
"""
...
def _rel_key(rel: ConcernRelationType | str) -> str:
    ...
    Mirrors :func:`opencoat_runtime_storage.memory.dcn_store._rel_key` so
...
class SqliteDCNStore(DCNStore):
    ...
    Example
    -------
    >>> from opencoat_runtime_storage.sqlite import SqliteDCNStore
    >>> dcn = SqliteDCNStore("/var/lib/opencoat/dcn.db")
