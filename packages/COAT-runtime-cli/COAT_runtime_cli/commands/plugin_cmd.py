...
files into ``--out`` (default ``./coat_plugin``) so a user can wire a
new host adapter in under a minute.

The starter files live under :mod:`COAT_runtime_cli.plugin_templates`
...
_DEFAULT_OUT_DIR = Path("coat_plugin")


def _write_package_init(dst: Path, host: str) -> None:
    dst.write_text(
        f'"""COAT host plugin scaffold — generated for {host!r}."""\n',
        encoding="utf-8",
    )
...
def _copy_templates(host: str, out: Path, *, force: bool) -> int:
    ...
    if host == "openclaw":
        ...
        print("  2. Call bootstrap_coat.install(your_openclaw_host) from your app.")
    else:
...
def register(sub: argparse._SubParsersAction) -> None:
    plugin = sub.add_parser(
        ...
        description="Scaffold and manage COAT host plugins.",
    )
    ...
    install = actions.add_parser(
        ...
        description=(
            ...
            "(__init__.py + bootstrap_coat.py + host_adapter.py + concerns.py) "
