...
import pytest
from COAT_runtime_cli.commands import plugin_cmd
from COAT_runtime_cli.main import main as cli_main

EXPECTED_FILES = ("__init__.py", "bootstrap_coat.py", "host_adapter.py", "concerns.py")


def _scaffold_dir(tmp_path: Path, host: str) -> Path:
    out = tmp_path / f"coat_plugin_{host}"
...
class TestPluginInstall:
    ...
    def test_install_default_out_dir(
        ...
        """Without ``--out`` we land in ``./coat_plugin`` relative to cwd."""
        monkeypatch.chdir(tmp_path)
        rc = cli_main(["--no-banner", "plugin", "install", "openclaw"])
        assert rc == 0
        target = tmp_path / "coat_plugin"
        for name in EXPECTED_FILES:
    ...
    def test_install_partial_failure_writes_nothing(self, tmp_path: Path) -> None:
        ...
        rc = cli_main(["--no-banner", "plugin", "install", "custom", "--out", str(out)])
        assert rc == 1
        # We must NOT have written __init__.py / bootstrap_coat.py / concerns.py.
        assert not (out / "__init__.py").exists()
        assert not (out / "bootstrap_coat.py").exists()
