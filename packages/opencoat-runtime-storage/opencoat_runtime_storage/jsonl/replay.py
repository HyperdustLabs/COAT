"""Replay a JSONL session against a :class:`OpenCOATRuntime` (M3 PR-15).
...
plus a deterministic :class:`~opencoat_runtime_core.llm.StubLLMClient`.
...
from opencoat_runtime_protocol import Concern, ConcernInjection
...
def build_runtime_for_replay(concerns: list[Concern]) -> OpenCOATRuntime:
...
def replay_parsed_session(runtime: OpenCOATRuntime, session: ParsedSession) -> ReplayResult:
